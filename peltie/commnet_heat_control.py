...
load_dotenv('.env')  # .envファイルから環境変数をロード

# 定数
INITIAL_TIME = 1000  # 初期遅延時間（ミリ秒）
RECORDING_TIME = 60000  # 録画時間（ミリ秒）

# ディレクトリ用のグローバル変数
Serial_Port = ''
LOG_DIR = r"C:\Users\otake\OneDrive - 東京理科大学 (1)\mause実験\ハンドリング20240521"
OBS_DIR = r"C:\Users\otake\OneDrive - 東京理科大学 (1)\mause実験\ハンドリング20240521"

# トリガーイベントを処理する関数
def trigger(event):
    global Serial_Port
    serial_open()  # シリアルポートを開く

    time.sleep(1)

    # シリアルポートから読み取るスレッドを開始
    ...
    # コマンドをスケジュール
    root.after(t1, command_w)  # 刺激開始とログ開始
    if stm.get():
        root.after(t2, command_x)  # 60秒後に刺激開始
        root.after(t3, command_y)  # 75秒後に刺激終了
    root.after(t4, command_z)  # 135秒後にログ終了
    root.after(t5, command_close)  # 135秒後にログ終了

# 最初のカメラ用のOBS録画を処理する関数
def Recording():
    global ws
...
# フォルダを作成する関数
def create_folder():
    base_path = OBS_DIR  # ベースディレクトリのパス
...
# シリアルポートにコマンド'w'を送信する関数
def command_w():
    global Serial_Port
    Serial_Port.write('w\r\n'.encode('utf-8'))

# シリアルポートにコマンド'x'を送信する関数
def command_x():
    global Serial_Port
    str1 = 'x' + stim_combo.get() + '\r\n'
    Serial_Port.write(str1.encode('utf-8'))

# シリアルポートにコマンド'y'を送信する関数
def command_y():
    global Serial_Port
    Serial_Port.write('y\r\n'.encode('utf-8'))

# シリアルポートにコマンド'z'を送信する関数
def command_z():
    global Serial_Port
    Serial_Port.write('z\r\n'.encode('utf-8'))
...
def serial_read():
    global Serial_Port
    # 0秒: 115, 5秒: 122, 10秒: 127, 15秒: 131, 20秒: 136, 25秒: 141, 30秒: 146
...
# スクロールテキストウィジェットの設定
st = scrolledtext.ScrolledText(root, width=40, height=15, font=("Helvetica", 15))
st.pack()
...
try:
    ...
    obspath = ''
    obsstate = 'disable'
    print(f'OBS 1 websocket is not connected: {e}')
...
try:
    ...
    obspath2 = ''
    obsstate2 = 'disable'
    print(f'OBS 2 websocket is not connected: {e}')
...
start_button = ttk.Button(root, text="実験スタート", width=50)
start_button.bind("<Button-1>", trigger)
start_button.pack()
